...
mkdir -p /scratch/okraus/.cache/wandb

if [ -f "/nethome/okraus/.wandb_api_key" ]; then
...
$PY_CMD "${PROJECT_ROOT}/src/lengthgen/results/select_runs.py" \
    --config "$CONFIG" 
$PY_CMD "${PROJECT_ROOT}/src/lengthgen/results/eval_test.py" \
    --config "$CONFIG"
$PY_CMD "${PROJECT_ROOT}/src/lengthgen/results/plot.py" \
    --config "$CONFIG"
