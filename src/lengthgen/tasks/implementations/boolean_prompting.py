...
def _maybe_replace_symbols(text: str):
...
def _boolean_generator(
    ...
    for _ in tqdm(range(num_samples), desc="Creating boolean dataset"):
        ...
        full_text = (
            f"{prompt_formula} {PROMPTING_TRACE_TOKEN} {trace_str} "
            f"{FINAL_ANSWER_PROMPTING_TOKEN} {answer_str} {END_OF_TEXT_PROMPTING_TOKEN}"
        )
        if use_word_operators:
            full_text = _maybe_replace_symbols(full_text)
        yield {"text": full_text}


def generate_boolean_prompting_dataset(
    num_samples: int,
    ...
    dataset = Dataset.from_generator(
        ...
        gen_kwargs={
            ...
            "include_indices": include_indices,
            "use_word_operators": use_word_operators
        },
        keep_in_memory=True
    )
...
def update_yaml_prompts(
    ...
    baseline_ds = generate_boolean_prompting_dataset(
    ...
    index_ds = generate_boolean_prompting_dataset(
        num_samples=num_shots,
        min_length=min_len,
        max_length=max_len,
        seed=seed,
...
if __name__ == "__main__":
    ds = generate_boolean_prompting_dataset(
    ...
    update_yaml_prompts("path/to/boolean_prompting.yaml")
